...
- **كتالوج المكونات** — `byoh catalog index` يستكشف [awesomeclaudeplugins.com](https://awesomeclaudeplugins.com) (أكثر من 24 000 مكوّن عبر `sitemap.xml`) ويحفظ ذاكرة تخزين مؤقت في `~/.byoh/catalog.json`. تُحلَّل كل صفحة أولاً من JSON-LD، مع التراجع إلى وسوم `<title>` + `<meta>` عند غيابه. بعد الفهرسة يعمل `catalog search` و`catalog vendor` بالكامل بدون اتصال. خلال مقابلة المعالج S2، يُضمّن `profile_interview` تلقائيًا `catalog_suggestions` — ما يصل إلى 5 مكونات مطابقة للنوع يمكن للـ LLM التوصية بها دون استدعاءات أدوات إضافية.
