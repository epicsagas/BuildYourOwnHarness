...
### وكيل الذكاء الاصطناعي الخاص بك، مصمم حولك
...
معظم أدوات الذكاء الاصطناعي تمنحك مجموعة ثابتة من الأدوات وتقول لك "بال توفيق." يقلب BYOH هذه المعادلة: يقابلك، يتعلّم ما تفعلله فعلًا، ثم يولّد هارنس وكيل مخصّصًا — مهارات وذاكرة وخطوط معالجة — يناسب طريقة عملك من اللحظة الأولى.
...
إن خطر ببالك يومًا "ليت ذكائي الاصطناعي يفهم سياقي فعلًا" — فهذا بالضبط ما يقدّمه BYOH.

## كيف يعمل في 60 ثانية

صُمِّم BYOH ليُقاد بواسطة وكيل الذكاء الاصطناعي لديك — لا بكتابتك للأوامر. ثبّت الإضافة، ثم تحدّث فقط. المحادثة *هي* المقابلة والمعالج والبناء في آنٍ واحد.
...
في الجلسة التالية يحمّل خادمك الهارنس تلقائيًا — وكلاء ومهارات وذاكرة وخطوط معالجة، كلها مضبوطة لك.

## ثبّت الإضافة (موصى به)

هل تستخدم **Claude Code أو Codex أو agy**؟ ثبّت الإضافة. فهي تجمع خادم MCP **وتُثبّت البرنامج الثنائي تلقائيًا عند أول تحميل** — دون سلسلة أدوات Rust ودون إعداد يدوي:
...
يتحدّث BYOH لغة MCP، لذا يعمل مع Cursor وZed وContinue وغيرها. ثبّت [الملف الثنائي](#installation) مرة واحدة، ثم وجّه خادمك نحو الخادم:
...
بمجرد وصل خادمك، أنت لا تكتب الأوامر — أنت تتحدّث فقط. يستدعي وكيلك أدوات BYOH MCP مباشرةً، والمحادثة *هي* المقابلة والبناء ودورة التطور:

> **أنت:** *أنا مطوّر Go خلفيّ أُطلق واجهة برمجية للمدفوعات هذا الشهر. ابنِ لي هارنس.*
>
> **الوكيل:** *(يفحص مستودعك عبر `profile_scan`، ويطرح أسئلة موجّهة عبر `profile_interview`، ثم يُثبّت التصنيف على `developer`)* ← يُجمّع `HarnessBundle` ← يُثبّت وكلاء ومهارات وذاكرة وخط شحن آمن في Claude Code. انتهى — في الجلسة التالية، وكيلك يتحدث لغتك التقنية.
...
يسألك BYOH عن دورك ومستوى خبرتك وأدواتك وهدفك لمدة 30 يومًا. تتكيّف المقابلة — فالباحث يحصل على أسئلة مختلفة عن المطور. يُشغّل `evolve` دورة ثلاثية البوابات (Critic / Seesaw / Stagnation) لا يمكن تجاوزها أبدًا — فالتطور آمن وقابل للمراجعة.
