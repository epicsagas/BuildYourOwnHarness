...
- **Plugin-Katalog** — `byoh catalog index` crawlt [awesomeclaudeplugins.com](https://awesomeclaudeplugins.com) (über 24 000 Plugins via `sitemap.xml`) und speichert einen Offline-Cache unter `~/.byoh/catalog.json`. Jede Seite wird primär aus JSON-LD geparst und greift andernfalls auf `<title>` + `<meta>`-Tags zurück. Nach der Indizierung funktionieren `catalog search` und `catalog vendor` vollständig offline. Während des S2-Wizard-Interviews fügt `profile_interview` automatisch `catalog_suggestions` hinzu — bis zu 5 genre-passende Plugins, die das LLM ohne zusätzliche Tool-Aufrufe empfehlen kann.
