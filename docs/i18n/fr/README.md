...
- **Catalogue de plugins** — `byoh catalog index` explore [awesomeclaudeplugins.com](https://awesomeclaudeplugins.com) (plus de 24 000 plugins via `sitemap.xml`) et sauvegarde un cache hors ligne dans `~/.byoh/catalog.json`. Chaque page est d'abord analysée depuis le JSON-LD, avec repli sur les balises `<title>` + `<meta>` sinon. Après l'indexation, `catalog search` et `catalog vendor` fonctionnent entièrement hors ligne. Lors de l'entretien S2 du wizard, `profile_interview` inclut automatiquement `catalog_suggestions` — jusqu'à 5 plugins correspondant au genre que le LLM peut recommander sans appels d'outils supplémentaires.
