...
- **Catalogue de plugins** — `byoh catalog index` (nécessite `--features catalog`) explore [awesomeclaudeplugins.com](https://awesomeclaudeplugins.com) (plus de 24 000 plugins via `sitemap.xml` + JSON-LD) et sauvegarde un cache hors ligne dans `~/.byoh/catalog.json`. Ensuite, `catalog search` et `catalog vendor` fonctionnent entièrement hors ligne. Lors de l'entretien S2 du wizard, `profile_interview` inclut automatiquement `catalog_suggestions` — jusqu'à 5 plugins correspondant au genre que le LLM peut recommander sans appels d'outils supplémentaires.
