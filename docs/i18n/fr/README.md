...
byoh catalog index [--limit N]           # explorer awesomeclaudeplugins.com → ~/.byoh/catalog.json
...
- **Catalogue de plugins** — `byoh catalog index` construit un cache hors ligne dans `~/.byoh/catalog.json` à partir de [awesomeclaudeplugins.com](https://awesomeclaudeplugins.com) (plus de 24 000 plugins via `sitemap.xml` ; chaque page est d'abord analysée depuis le JSON-LD, avec repli sur les balises `<title>` + `<meta>` sinon). Par défaut, il télécharge d'abord un **bundle préconstruit par le mainteneur** (un asset GitHub Release hebdomadaire — quelques secondes, sans crawl) et ne explore le site lui-même que si celui-ci est indisponible. Après l'indexation, `catalog search` et `catalog vendor` fonctionnent entièrement hors ligne. Lors de l'entretien S2 du wizard, `profile_interview` inclut automatiquement `catalog_suggestions` — jusqu'à 5 plugins correspondant au genre que le LLM peut recommander sans appels d'outils supplémentaires.
