...
- **Catálogo de plugins** — `byoh catalog index` rastrea [awesomeclaudeplugins.com](https://awesomeclaudeplugins.com) (más de 24 000 plugins via `sitemap.xml`) y guarda un caché offline en `~/.byoh/catalog.json`. Cada página se analiza primero desde JSON-LD y, si no está presente, se recurre a las etiquetas `<title>` + `<meta>`. Tras la indexación, `catalog search` y `catalog vendor` funcionan completamente sin conexión. Durante la entrevista del asistente S2, `profile_interview` incluye automáticamente `catalog_suggestions` — hasta 5 plugins coincidentes por género que el LLM puede recomendar sin llamadas adicionales a herramientas.
